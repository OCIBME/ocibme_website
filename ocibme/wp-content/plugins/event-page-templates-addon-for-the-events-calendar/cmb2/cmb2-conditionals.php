...
if ( ! class_exists( 'CMB2_Conditionals', false ) ) {
	...
	class CMB2_Conditionals {
		...
		// array of custom post types where CMB2 and CMB2 conditionals are required;
		if( class_exists('cool_plugins_extras') ){
			$extras = new cool_plugins_extras();
			
					if( $extras->cmb2_get_post_type_page() ){
						wp_enqueue_script(
							'cmb2-conditionals',
							plugins_url( 'js/cmb2-conditionals.js', __FILE__ ),
...
if( !class_exists( 'cool_plugins_extras') ){
	class cool_plugins_extras{
		public function cmb2_get_post_type_page() {
			global $post, $typenow, $current_screen;
			$custom_post = array('epta');
			$enable_posts = apply_filters('cmb2_condition_based_enqueue',$custom_post );
			 if ( $post && $post->post_type ){
					return in_array($post->post_type, $enable_posts );
			 }elseif( $typenow ){
					return in_array($typenow, $enable_posts );
			  }elseif( $current_screen && $current_screen->post_type ){
					return in_array($current_screen->post_type, $enable_posts );
			 }
			 elseif( isset( $_REQUEST['post_type'] ) ){
					return in_array(sanitize_key( $_REQUEST['post_type'] ), $enable_posts );
			 }
			 elseif ( isset( $_REQUEST['post'] ) ) {
			   return in_array(get_post_type( $_REQUEST['post'] ), $enable_posts );
			 }
			  return null;
		}
	}
