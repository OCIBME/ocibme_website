...
function epta_share_button($event_id){
	...
	// Construct sharing URL
		...
		$tecset_sharecontent .= '<a class="tecset-share-link" href="'.$tecset_facebookurl.'" target="_blank" title="Facebook" aria-haspopup="true"><i class="ect-icon-facebook"></i></a>';
		$tecset_sharecontent .= '<a class="tecset-share-link" href="'.$tecset_twitterURL.'" target="_blank" title="Twitter" aria-haspopup="true"><i class="ect-icon-twitter"></i></a>';
		...
		$tecset_sharecontent .= '<a class="tecset-email" href="'.$tecset_emailUrl.' "title="Email" aria-haspopup="true"><i class="ect-icon-mail"></i></a>';
		$tecset_sharecontent .= '<a class="tecset-share-link" href="'.$tecset_whatsappURL.'" target="_blank" title="WhatsApp" aria-haspopup="true"><i class="ect-icon-whatsapp"></i></a>';
...
function epta_event_schedule($event_id,$tecset_date_format){
	...
	else if($tecset_date_format=="FD,Y") {
			$tecset_event_schedule='<div class="tecset-date"  itemprop="startDate" content="'.tribe_get_start_date($event_id, false, 'Y-m-dTg:i').'">';
			if (!tribe_event_is_multiday( $event_id ) ) {
				$tecset_event_schedule.='<span class="tecset-ev-mo">'.tribe_get_start_date($event_id, false, 'F' ).'</span>
				<span class="tecset-ev-day">'.tribe_get_start_date($event_id, false, 'd' ).', </span>
	...
	else if($tecset_date_format=="MD,Y") {
			$tecset_event_schedule='<div class="tecset-date"  itemprop="startDate" content="'.tribe_get_start_date($event_id, false, 'Y-m-dTg:i').'">';
			if (!tribe_event_is_multiday( $event_id ) ) {
				$tecset_event_schedule.='<span class="tecset-ev-mo">'.tribe_get_start_date($event_id, false, 'M' ).'</span>
				<span class="tecset-ev-day">'.tribe_get_start_date($event_id, false, 'd' ).', </span>
	...
	else if($tecset_date_format=="MD,YT") {
			$tecset_event_schedule='<div class="tecset-date" itemprop="startDate" content="'.tribe_get_start_date($event_id, false, 'Y-m-dTg:i').'">';
			if (!tribe_event_is_multiday( $event_id ) ) {
				$tecset_event_schedule.='<span class="tecset-ev-mo">'.tribe_get_start_date($event_id, false, 'M' ).'</span>
				<span class="tecset-ev-day">'.tribe_get_start_date($event_id, false, 'd' ).', </span>
	...
	else if($tecset_date_format=="full") {
			$tecset_event_schedule='<div class="tecset-date" itemprop="startDate" content="'.tribe_get_start_date($event_id, false, 'Y-m-dTg:i').'">';								
				if (!tribe_event_is_multiday( $event_id ) ) {
					$tecset_event_schedule.='<span class="tecset-ev-day">'.tribe_get_start_date($event_id, false, 'd' ).'</span>
					<span class="tecset-ev-mo">'.tribe_get_start_date($event_id, false, 'F' ).'</span>
	...
	else if($tecset_date_format=="dFY"){
			$tecset_event_schedule='<div class="tecset-date" itemprop="startDate" content="'.tribe_get_start_date($event_id, false, 'Y-m-dTg:i').'">';								
			if (!tribe_event_is_multiday( $event_id ) ) {
				$tecset_event_schedule.='<span class="tecset-ev-day">'.tribe_get_start_date($event_id, false, 'd' ).'</span>
				<span class="tecset-ev-mo">'.tribe_get_start_date($event_id, false, 'F' ).'</span>
	...
	else if($tecset_date_format=='dMY') {
			$tecset_event_schedule='<div class="tecset-date" itemprop="startDate" content="'.tribe_get_start_date($event_id, false, 'Y-m-dTg:i').'">';								
			if (!tribe_event_is_multiday( $event_id ) ) {
				$tecset_event_schedule.='<span class="tecset-ev-day">'.tribe_get_start_date($event_id, false, 'd' ).'</span>
				<span class="tecset-ev-mo">'.tribe_get_start_date($event_id, false, 'M' ).'</span>
...
function epta_tribe_event_time($post_id, $display = true ) {
			...
			elseif ( tribe_event_is_multiday( $event ) ) { // multi-date event
					...
					if ( $display ) {
							printf( __( '%s - %s', 'ect' ), $tecset_start_date, $tecset_end_date );
					}
					else {
							return sprintf( __( '%s - %s', 'ect' ), $tecset_start_date, $tecset_end_date );
					}
			}
			else {
					...
					if ( $tecset_start_date!== $tecset_end_date ) {
							...
							if ( $display ) {
									printf( '%s', $tecset_start_date);
							}
							else {
									return sprintf( '%s', $tecset_start_date);
							}
					}
			}
	}
